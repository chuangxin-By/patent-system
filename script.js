...
async function saveToLocalStorage() {
    try {
        ...
        if (window.db) {
            // 先删除所有记录
            await window.db.from('patents').delete().neq('id', 0);
            
            // 插入新记录
            for (const patent of patents) {
                await window.db.from('patents').insert({
                    id: patent.id,
                    patent_name: patent.patentName,
                    patent_type: patent.patentType,
                    inventor_details: JSON.stringify(patent.inventorDetails),
                    image: patent.image
                });
            }
            ...
            const amounts = {
                id: 1,
                invention_amount: parseFloat(document.getElementById('inventionAmount').value),
                utility_amount: parseFloat(document.getElementById('utilityAmount').value),
                design_amount: parseFloat(document.getElementById('designAmount').value)
            };

            await window.db.from('settings')
                .upsert(amounts, { onConflict: 'id' });
        }
    } catch (e) {
