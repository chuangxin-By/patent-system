...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 添加金额输入框变化监听
    ['inventionAmount', 'utilityAmount', 'designAmount'].forEach(id => {
        document.getElementById(id).addEventListener('change', () => {
            if (document.getElementById('dataTable').style.display !== 'none') {
                updateTable(); // 当金额改变时更新表格
            }
        });
    });
    ...
    try {
        ...
        if (window.db) {
            const amountsRef = window.db.ref(window.db.database, 'amounts');
            window.db.get(amountsRef).then((snapshot) => {
                const cloudAmounts = snapshot.val();
                if (cloudAmounts) {
                    document.getElementById('inventionAmount').value = cloudAmounts.invention;
                    document.getElementById('utilityAmount').value = cloudAmounts.utility;
                    document.getElementById('designAmount').value = cloudAmounts.design;
                }
            });
        }
    } catch (e) {
        console.error('加载金额设置时出错：', e);
    }
});
...
function saveToLocalStorage() {
    try {
        ...
        localStorage.setItem('patentAmounts', JSON.stringify(amounts));
        ...
        if (window.db) {
            ...
            const amountsRef = window.db.ref(window.db.database, 'amounts');
            window.db.set(amountsRef, amounts);
        }
    } catch (e) {
